...
* **Windows 远端方向键历史**：会话建立时显式发送与 OpenSSH 一致的标准终端模式（term_modes：ECHO/ICANON/ICRNL/ONLCR 等）。asyncssh 默认不发送 term_modes，会导致 Windows sshd 未正确初始化远端控制台，出现"↑ 上一条命令后 ↓ 无法返回"的 PSReadLine 异常——补上后与原生 ssh 行为一致。
...
系统记录用户通过 Web SSH Terminal 执行过的命令，作为**轻量辅助**（真正的高价值复用功能是 §10 命令备忘/命令库）。
...
* **同服务器同命令去重**：重复执行的命令只保留最近一次，避免高频命令刷屏；
* **保留最近 200 条**（不再无限累积）；
* 记录仍为启发式（Enter 时从终端屏幕行读取，含 Tab 补全结果；无法可靠识别时回退输入缓存）。
