...
description: 项目的测试策略。新增/修改功能、修 bug、写测试、跑测试前必读。含测试分层、bug 先红后绿流程、前端 bug 分层测试原则、Playwright E2E 策略、AI 协作注意。
whenToUse: 涉及测试时（写测试、跑测试、修 bug 需复现、E2E 验证、报告测试结果前）。
---
...
1. **单元/集成可复现**（逻辑、API、组件交互）：
   ...
   - bug 改变对外行为 → 按 `feature-development` skill 更新文档。
